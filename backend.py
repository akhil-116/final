...
class RequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def upload_encrypted_file(self):
        ...
        file_name = self.headers['X-File-Name']  # Retrieve original filename from request
        #file_name = secrets.token_hex(16)  # Generating a random filename

        # Store the uploaded encrypted file specific to the user
        ...
        aes_key = self.headers['Aes-Key']
        aes_iv = self.headers['Aes-Iv']
    ...
    def decrypt_and_download(self):
        ...
        # Check if the file is encrypted
        cursor.execute('SELECT is_encrypted FROM encrypted_files WHERE filename = ? AND username = ?', (file_name, user_cookie))
        result = cursor.fetchone()
        if not result:
            self.send_response(HTTPStatus.BAD_REQUEST)
            self.end_headers()
            self.wfile.write(b'File not found in the encrypted files table.')
            return
        ...
        if is_encrypted:
            ...
            self.send_response(HTTPStatus.BAD_REQUEST)
            self.end_headers()
            self.wfile.write(b'File is not encrypted. If you want to download the unencrypted file, please download it from the uploaded files.')
    
    '''
    def connect_vpn(self):
        # Define the VPN server's IP address and port
        VPN_SERVER_HOST = '127.0.0.1'  # Update with your VPN server's IP
        VPN_SERVER_PORT = 12345         # Update with your VPN server's port

        # Connect to the VPN server
        try:
            self.vpn_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.vpn_socket.connect((VPN_SERVER_HOST, VPN_SERVER_PORT))
            self.send_response(200)
            self.end_headers()
            ...
            self.end_headers()
            self.wfile.write(f"Error connecting to VPN server: {str(e)}".encode())

    def disconnect_vpn(self):
        if hasattr(self, 'vpn_socket') and self.vpn_socket is not None:  # Check if vpn_socket is initialized
            try:
                self.vpn_socket.close()
                self.send_response(200)
                self.end_headers()
                self.wfile.write(b"Disconnected from VPN server")
            except Exception as e:
                self.send_response(500)
                self.end_headers()
                self.wfile.write(f"Error disconnecting from VPN server: {str(e)}".encode())
        else:
            self.send_response(500)
            self.end_headers()
            self.wfile.write(b"No VPN connection to disconnect")
    '''
    ...
    def connect_vpn(self):
        """
        Method to establish a VPN connection.
        """
        openvpn_executable = "C:\\Program Files\\OpenVPN\\bin\\openvpn.exe"
        client_config_file = "client.ovpn"  # Specify the path to your .ovpn file
    
        
        try:
            # Command to initiate the VPN connection using the OpenVPN client
            command = [openvpn_executable, "--config", client_config_file]
    ...
    def disconnect_vpn(self):
        """
        Method to disconnect from a VPN connection.
        """
        openvpn_executable = "C:\\Program Files\\OpenVPN\\bin\\openvpn.exe"
        ovpn_path = "client.ovpn"  # Specify the path to your .ovpn file

        try:
            # Use subprocess to run the OpenVPN command to disconnect from the VPN
            subprocess.run([openvpn_executable, "--config", ovpn_path, "--down", "default"], check=True)
            
            # Send success response
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            response = {'message': 'VPN disconnected successfully.'}
        except Exception as e:
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            response = {'message': f'Failed to disconnect VPN: {str(e)}'}
    ...
    def connect_vpn(self):
        try:
            # Command to execute OpenVPN with the specified configuration file
            config_file_path = "C:\Program Files\OpenVPN\config\myserver.ovpn"  #  your actual path
            command = ["openvpn", "--config", config_file_path]
            
            # Execute the command
            subprocess.run(command, check=True)
            
            # If execution reaches here, VPN connected successfully
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            response = {'message': 'VPN connected successfully.'}
            self.wfile.write(json.dumps(response).encode('utf-8'))
        except subprocess.CalledProcessError as e:
    ...
    def disconnect_vpn(self):
        try:
            # Terminate any running OpenVPN processes to disconnect from the VPN
            subprocess.run(["pkill", "-f", "openvpn"], check=True)
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            response = {'message': 'VPN disconnected successfully.'}
