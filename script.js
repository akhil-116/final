...
function logoutUser() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                window.location.href = 'demo.html'; // Redirect after logout
            } else {
...
function resetFileInputs() {
    document.getElementById('fileInput').value = ''; // Reset the value of file input field for uploading files
    document.getElementById('fileToSend').value = ''; // Reset the value of file input field for sending files
}
...
function decryptAndDownload() {
    ...
    var aesKeyInput = document.getElementById('aesKeyInput');
    var aesIVInput = document.getElementById('aesIvInput');
    ...
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'https://localhost:8443/decrypt_and_download', true);
    xhr.setRequestHeader('X-File-Name', file.name);
...
function connectVPN() {
    ...
    xhr.open('POST', '/connect_vpn', true);
    xhr.setRequestHeader('Content-Type', 'application/json');
